...
export default function CallbackTable() {
  ...
  const deleteLead = async (id: number) => {
    ...
    await fetch("/api/delete-callback", {
      method: "POST",
  ...
  const saveEdit  = async () => {
    ...
    setEditError("")
    try {
      const res = await fetch("/api/edit-callback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const exportCSV = (rows: Lead[]) => {
    const csv =
      ...
      rows
        .map((l) => [q(l.name), q(l.phone), q(l.email), q(fmtDate(l.created_at))].join(","))
        .join("\n")
    const a = Object.assign(document.createElement("a"), {
      href: URL.createObjectURL(new Blob([csv], { type: "text/csv" })),
      download: `callback-requests-${toInputDate(new Date())}.csv`,
    })
    a.click()
  }
  ...
  const exportLabel   = selected.size > 0
    ? `Export (${selected.size})`
    : hasDateFilter
    ? `Export Filtered (${filtered.length})`
    : `Export All (${filtered.length})`

  const headers = ["", "Name", "Phone", "Email", "Date", "Actions"]
    ...
    <div className="p-4 sm:p-6">
      ...
      {someSelected && (
        <div className="mb-4 flex items-center justify-between flex-wrap gap-3 px-4 py-3 rounded-xl bg-amber-50 border border-primary/30">
          <div className="flex items-center gap-2 text-sm font-semibold text-foreground flex-wrap">
            <span className="w-7 h-7 flex items-center justify-center rounded-lg bg-primary text-black text-xs font-black">
              {selected.size}
            </span>
            ...
            <button
              onClick={selectAll}
              className="text-xs text-accent hover:text-primary underline underline-offset-2 transition-colors"
            >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            {hasDateFilter && (
              <span className="ml-0.5 w-4 h-4 flex items-center justify-center rounded-full bg-black/20 text-[9px] font-black">
                ✓
              </span>
            )}
          </button>
      ...
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <p className="text-xs text-muted-foreground">
          ...
          lead{filtered.length !== 1 ? "s" : ""} found
        </p>
            ...
            {!loading && paginated.map((lead) => (
              <tr
                ...
                className={`border-b border-border last:border-0 transition-colors ${
                  selected.has(lead.id) ? "bg-primary/5" : "hover:bg-muted/40"
                }`}
              >
                ...
                <td className="px-4 py-3.5">
                  <div className="flex items-center gap-1.5">
                    <a
                      href={`https://wa.me/${lead.phone.replace(/\D/g, "")}?text=Hello%20${encodeURIComponent(lead.name)}%2C%20we%20received%20your%20callback%20request.`}
                      target="_blank"
        ...
        <div className="mt-5 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={() => setPage(1)}
              disabled={safePage === 1}
              className="px-2.5 py-1.5 rounded-lg border border-border text-xs font-semibold text-muted-foreground hover:bg-muted disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
              «
            </button>
            <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md bg-background border border-border rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              {editError && (
                <p className="px-3 py-2 rounded-xl bg-red-50 border border-red-200 text-red-600 text-xs font-medium">
                  {editError}
                </p>
              )}
            </div>
