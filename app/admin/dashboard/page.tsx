import AdminLogoButton from "@/components/admin/AdminLogoButton"
...
async function getLeadsData(): Promise<{ stats: { total: number; thisMonth: number; thisWeek: number; today: number }, leads: Lead[] }> {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/api/leads`, {
      cache: "no-store",
    })
    if (!res.ok) return { stats: { total: 0, thisMonth: 0, thisWeek: 0, today: 0 }, leads: [] }
    ...
    return {
      ...
      stats: {
        total:     leads.length,
        today:     leads.filter(l => l.submitted_at && new Date(l.submitted_at) >= todayStart).length,
        thisWeek:  leads.filter(l => l.submitted_at && new Date(l.submitted_at) >= weekStart).length,
        thisMonth: leads.filter(l => l.submitted_at && new Date(l.submitted_at) >= monthStart).length,
      }
    }
  } catch (err) {
    console.error("[Dashboard] Stats error:", err)
    return { stats: { total: 0, thisMonth: 0, thisWeek: 0, today: 0 }, leads: [] }
  }
}

export default async function Page() {
  const [{ stats, leads }, logoUrl] = await Promise.all([getLeadsData(), getLogoUrl()])

  const statCards = [
    { label: "Total Leads", value: stats.total,     icon: HiOutlineClipboardList, hint: "All time" },
    { label: "This Month",  value: stats.thisMonth,  icon: HiOutlineTrendingUp,    hint: new Date().toLocaleString("en-IN", { month: "long", year: "numeric" }) },
    { label: "This Week",   value: stats.thisWeek,   icon: HiOutlineCalendar,      hint: "Mon → today" },
    { label: "Today",       value: stats.today,      icon: HiOutlineClock,         hint: new Date().toLocaleDateString("en-IN", { day: "numeric", month: "short" }) },
  ]

  return (
    <div className="relative min-h-svh bg-background text-foreground flex flex-col">
      ...
      {/* ── Ambient glows ── */}
      <div aria-hidden className="pointer-events-none fixed -top-24 -right-24 w-96 h-96 rounded-full z-0"
        style={{ background: "radial-gradient(circle, rgba(246,185,19,0.1) 0%, transparent 70%)", filter: "blur(60px)" }} />
      <div aria-hidden className="pointer-events-none fixed -bottom-24 -left-20 w-80 h-80 rounded-full z-0"
      ...
      <header className="sticky top-0 z-50 bg-background/90 backdrop-blur-lg border-b border-border shadow-sm">
        <div
          ...
          style={{ background: "linear-gradient(90deg, transparent, rgba(246,185,19,0.35) 10%, #F6B913 35%, #ffe55a 50%, #F6B913 65%, rgba(246,185,19,0.35) 90%, transparent)" }}
        />
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10 h-16 flex items-center justify-between">

          <AdminLogoButton logoUrl={logoUrl} />
      ...
      {/* ══════════════════════════════════════
          MAIN
      ══════════════════════════════════════ */}
      <main className="relative z-10 flex-1 overflow-x-hidden mx-auto w-full max-w-[1400px] px-4 sm:px-6 lg:px-10 py-8 sm:py-10 lg:py-12">

        {/* Page Header */}
        <div className="mb-8 sm:mb-10">
          <div className="flex items-center gap-2 mb-2">
            <span className="w-5 h-0.5 rounded-full bg-primary" />
            <span className="text-[10px] font-bold tracking-[0.22em] uppercase text-accent">Admin Dashboard</span>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-8 sm:mb-10">
          {statCards.map(({ label, value, icon: Icon, hint }) => (
            ...
            >
              ...
              <div className="font-heading text-4xl font-black tracking-tight text-foreground leading-none mb-1">{value}</div>
              <div className="text-[10px] font-bold tracking-[0.16em] uppercase text-muted-foreground">{label}</div>
              {hint && <div className="text-[10px] text-muted-foreground/50 mt-0.5 truncate">{hint}</div>}
            </div>
          ))}
        </div>
        ...
        <div className="relative bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3 px-5 sm:px-6 py-4 bg-muted border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <span className="font-heading text-xl font-bold uppercase tracking-wide text-foreground">All Quote Requests</span>
            </div>
